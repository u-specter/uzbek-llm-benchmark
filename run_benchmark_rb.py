#!/usr/bin/env python3
"""
ULAB — Robustness Module (RB)
Tests models on intent detection with noisy Uzbek banking text.
Noise types: apostrophe_drop, typo, cyrillic_mix.
Scoring is fully automatic — no GPT-4o needed.

Usage:
    python run_benchmark_rb.py              # All questions, all models
    python run_benchmark_rb.py --model gpt-4o
    python run_benchmark_rb.py --type apostrophe_drop
    python run_benchmark_rb.py --ids RB-A-01,RB-T-03
"""
...
# Manual-only models (imported via import_manual.py --module rb)
MANUAL_MODEL_IDS = {"gemini-pro", "mistral-large", "deepseek-v3", "yandexgpt", "grok-3"}


# ─── Prompt ──────────────────────────────────────────────────────────────────

def build_rb_prompt(text: str, choices: list) -> str:
    choices_str = " / ".join(choices)
    return f"""Quyidagi mijoz xabarini o'qing va uning maqsadini aniqlang.
...
async def run_question(q: dict, models: list) -> dict:
    """Run one RB question against all models."""
    prompt = build_rb_prompt(q["text"], q["choices"])
...
def compute_rb_leaderboard(rb_results: list, model_ids: list) -> dict:
    """Compute per-noise-type and overall accuracy for RB."""
    noise_types = ["apostrophe_drop", "typo", "cyrillic_mix"]
    scores = {m: {"overall": [], **{nt: [] for nt in noise_types}} for m in model_ids}
    ...
    return {
        m: {
            ...
            "apostrophe_drop": avg(data["apostrophe_drop"]),
            "typo":            avg(data["typo"]),
            "cyrillic_mix":    avg(data["cyrillic_mix"]),
        }
...
async def main():
    parser = argparse.ArgumentParser(description="ULAB — RB Benchmark")
    parser.add_argument("--model",  help="Run only this model ID")
    parser.add_argument("--type",   help="Noise type: apostrophe_drop / typo / cyrillic_mix")
    parser.add_argument("--ids",    help="Comma-separated question IDs, e.g. RB-A-01,RB-T-03")
    parser.add_argument("--force",  action="store_true", help="Re-run even if response already exists")
    args = parser.parse_args()

    if not RB_QUESTIONS.exists():
        print(f"ERROR: {RB_QUESTIONS} not found.")
        sys.exit(1)
    ...
    print(f"\n{'═'*60}")
    print(f"  ULAB — Robustness Benchmark (RB)")
    print(f"{'═'*60}")
    ...
    for q in questions:
        ...
        if qid in existing_rb and not args.force:
            existing = existing_rb[qid]
        ...
        if qid in existing_rb:
            existing_rb[qid].setdefault("responses", {}).update(q_result["responses"])
        else:
            existing_rb[qid] = q_result
    ...
    all_rb_results = list(existing_rb.values())

    all_model_ids  = list({r for q in all_rb_results for r in q.get("responses", {})})
    rb_leaderboard = compute_rb_leaderboard(all_rb_results, all_model_ids)

    results["rb_questions"]   = all_rb_results
    results["rb_leaderboard"] = rb_leaderboard

    RESULTS_JSON.write_text(json.dumps(results, ensure_ascii=False, indent=2), encoding="utf-8")
    export_results_js(results)

    print(f"\n{'═'*60}")
    print(f"  RB Leaderboard")
    print(f"{'═'*60}")
    sorted_lb = sorted(rb_leaderboard.items(), key=lambda x: x[1]["overall"] or 0, reverse=True)
    for rank, (model_id, s) in enumerate(sorted_lb, 1):
        ov = s["overall"]
        ad = s["apostrophe_drop"]
        ty = s["typo"]
        cm = s["cyrillic_mix"]
        print(f"  {rank:>2}. {model_id:<22} Overall: {str(ov) + '%':>6}  |  Apost: {str(ad) + '%':>6}  Typo: {str(ty) + '%':>6}  Cyrillic: {str(cm) + '%':>6}")
