#!/usr/bin/env python3
"""
ULAB — Yuridik Q&A Module (YQA)
60 real banking/legal questions sourced from yuridik.uz (mrmuminov/yuridik-qa).
Models answer in Uzbek; GPT-4o compares against reference answer (0 / 50 / 100).

Scoring scale:
  100 — togri    (key facts match reference)
   50 — qisman   (partially correct)
    0 — notogri  (wrong or missing key facts)

Subtypes: finance, business, tax, consumer

Usage:
    python run_benchmark_yqa.py
    python run_benchmark_yqa.py --model gpt-4o
    python run_benchmark_yqa.py --type finance
    python run_benchmark_yqa.py --ids YQA-F-01,YQA-B-03
"""
...
YQA_QUESTIONS = BASE_DIR / "questions" / "yqa_questions.json"
...
def build_yqa_prompt(question: str) -> str:
    return f"""Sen O'zbek tilida moliyaviy, soliq va yuridik masalalar bo'yicha maslahat beruvchi assistantsan.

Savol: {question}

O'zbek tilida aniq va to'liq javob ber. Faqat savolga javob ber."""
...
def check_yqa_response(model_answer: str, verdict_raw: str) -> dict:
...
async def run_question(q: dict, models: list) -> dict:
    ...
    prompt    = build_yqa_prompt(question)
    ...
    for model_id, result in zip(tasks.keys(), raw):
        ...
        if result.get("error") or not result.get("response"):
            responses[model_id] = {"error": result.get("error", "empty_response"), "score": 0, "correct": False}
        else:
            responses[model_id] = {
...
def compute_yqa_leaderboard(yqa_results: list, model_ids: list) -> dict:
    subtypes = ["finance", "business", "tax", "consumer"]
    scores   = {m: {"overall": [], **{s: [] for s in subtypes}} for m in model_ids}

    for q in yqa_results:
        q_type = q.get("type", "")
...
async def main():
    parser = argparse.ArgumentParser(description="ULAB — YQA Benchmark (Yuridik Q&A)")
    parser.add_argument("--model", help="Run only this model ID")
    parser.add_argument("--type",  help="Subtype: finance / business / tax / consumer")
    parser.add_argument("--ids",   help="Comma-separated question IDs, e.g. YQA-F-01,YQA-B-03")
    parser.add_argument("--force", action="store_true", help="Re-run even if response already exists")
    args = parser.parse_args()

    if not YQA_QUESTIONS.exists():
        print(f"ERROR: {YQA_QUESTIONS} not found.")
        sys.exit(1)
    if not RESULTS_JSON.exists():
        print(f"ERROR: {RESULTS_JSON} not found.")
        sys.exit(1)

    yqa_data  = json.loads(YQA_QUESTIONS.read_text(encoding="utf-8"))
    questions = yqa_data["questions"]
    ...
    print(f"\n{'═'*60}")
    print(f"  ULAB — Yuridik Q&A Benchmark (YQA)")
    print(f"{'═'*60}")
    ...
    existing_yqa = {q["id"]: q for q in results.get("yqa_questions", [])}

    for q in questions:
        qid = q["id"]

        if qid in existing_yqa and not args.force:
            existing = existing_yqa[qid]
        ...
        if qid in existing_yqa:
            existing_yqa[qid].setdefault("responses", {}).update(q_result["responses"])
        else:
            existing_yqa[qid] = q_result
    ...
    all_yqa   = list(existing_yqa.values())
    model_ids = list({r for q in all_yqa for r in q.get("responses", {})})
    yqa_lb    = compute_yqa_leaderboard(all_yqa, model_ids)

    results["yqa_questions"]   = all_yqa
    results["yqa_leaderboard"] = yqa_lb

    RESULTS_JSON.write_text(json.dumps(results, ensure_ascii=False, indent=2), encoding="utf-8")
    export_results_js(results)

    print(f"\n{'═'*60}")
    print(f"  YQA Leaderboard")
    print(f"{'═'*60}")
    sorted_lb = sorted(yqa_lb.items(), key=lambda x: x[1].get("overall") or 0, reverse=True)
